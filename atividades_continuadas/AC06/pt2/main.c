...
int main() {
    ...
    while (true) {
        ...
        // ------ insert_after l i v --------
        else if(strcmp(token, "insert_after") == 0) {
            int l = atoi(strtok(NULL, " "));
            int i = atoi(strtok(NULL, " "));
            int v = atoi(strtok(NULL, " "));
            listas.data[l] = list_inserir_apos(listas.data[l], i, v);
        }

        // -------- remove_after l i --------
        else if(strcmp(token, "remove_after") == 0) {
            int l = atoi(strtok(NULL, " "));
            int i = atoi(strtok(NULL, " "));
            listas.data[l] = list_remover_apos(listas.data[l], i);
        }
