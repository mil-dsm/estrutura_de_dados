...
// Função que retorna o tamanho de uma árvore a partir da contagem de nós
// Recebe uma raiz como entrada e utiliza do persurso de pré-ordem para 
// retornar o resultado por meio de um inteiro
int arv_tamanho(No *node) {
    if(arv_vazia(node)) { // Caso base
        return 0;
    } else { // Caso geral
        return 1 + arv_tamanho(node->esq) + arv_tamanho(node->dir);
    }
}

// Função que retorna a altura de uma árvore a partir da contagem de nós
// Recebe uma raiz como entrada e utiliza do persurso de pós-ordem para 
// retornar o resultado por meio de um inteiro
int arv_altura(No *node) {
    if(arv_vazia(node)) { // Caso base
        return -1;
    } else { // Caso geral
        int sae = arv_altura(node->esq);
        int sad = arv_altura(node->dir);
